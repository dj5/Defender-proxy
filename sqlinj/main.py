...
from flask import Flask, jsonify, render_template, request, g, render_template_string
...
def loginAPI():
    if request.method == 'POST':
        ...
        if cur.fetchone():
            ...
            result = {'status': 'fail'}
        g.db.close()
        return render_template_string(result['status'])
...
def storeapi():
    if request.method == 'GET':
        ...
        g.db.commit()
        g.db.close()
        return render_template_string('name: '+name+'quantity: '+quan+'price: '+price)
...
if __name__ == "__main__":
    ...
    app.run(host='0.0.0.0',port =5050) # runs on machine ip address to make it visible on netowrk
